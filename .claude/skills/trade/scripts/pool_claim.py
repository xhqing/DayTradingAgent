...
def _norm_sym(s):
    """标的代码规范化：去空白、统一大写、裸数字补 HK. 前缀（HK.00700 / 00700 / 700 / us.mu 同义）。

    2026-09-01 修复：裸 5 位数字（01211）此前不补 HK. 前缀，与 HK.01211 分属两个键——
    同一标的被两会话重复认领（09:35 实录：会话2 认 01211、会话3 认 HK.01211，互斥失效）。
    同日冒烟再暴露短格式漏洞：3/4 位裸数字（700）不进「len==5」分支、也不进 HK. zfill
    分支，原样落键——裸数字一律先补 HK. 前缀（≤5 位，港股代码全数字）再 zfill 到 5 位，
    任意位数裸格式与富途格式同键。
    """
    s = s.strip().upper()
