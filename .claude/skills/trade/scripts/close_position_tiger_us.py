...
def main():
    ...
    if stp is not None:
        ...
        filled, fill_price, status, _reason = U.check_order_filled_us(config, stp_id, timeout=12)
        if filled:
            ...
            result_base.update({"ok": True, "order_id": stp_id, "fill_price": fill_price,
                                ...
                                "main_status": status})
            # 平仓成交后复查并撤掉全部残留止损单（2026-08-16 立，同港股版：残留兄弟止损单
            # 日后零持仓触发会反向开仓）
    ...
    filled, fill_price, status, _reason = U.check_order_filled_us(config, order_id, timeout=8)
    if not filled:
...
def _fallback_cancel_and_mo(config, symbol, close_side, quantity, result_base, current_price,
    ...
    ⚠️ 2026-08-07 事故修复（与港股 close_position_tiger.py 同步）：**下 MO 前必须先复查持仓**
    ——modify 触发价=现价后，止损单可能已实际触发成交（价格瞬时穿越触发价、轮询状态滞后
    误判「未触发」），此时持仓已被平；若再撤止损 + MO 按原量平仓，会**超卖反向开仓**
    （2026-08-07 港股 MINIMAX 实测复现：平多 20 股 → 反向开空 20 股）。修复：先查持仓——
    已空 → 只撤残留止损单、不下 MO；持仓剩余 < 原量 → MO 只平剩余量。
    """
    try:
        pos = U.get_open_position_us(config, symbol)
    except Exception:
