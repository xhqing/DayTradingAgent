...
给港股已有持仓加独立止损单 STP（aux_price=触发价，触发后市价成交；长桥对应 MIT）。
**先下新止损单、再撤旧止损单**（仓位持续有止损保护、无裸奔空窗——若先撤旧、新还没下，
仓位瞬间无保护），量严格=持仓量（超持仓券商判失效）。触发价取整到港股 tick。

✅ 实测状态（2026-08-03）：下单链路已 paper 开盘实测通过——独立 STP 止损单提交成功
（trigger 484.0 进入 HELD 监控）、「先新增后撤旧」工作正常、开仓附加腿落成的旧 STP 单可独立撤销
（腾讯 100 股）。
...
def main():
    ...
    try:
        n, ids = U.cancel_all_stop_orders_tiger(config, symbol, exclude_order_id=stop_order_id)
    ...
    result_base.update({"ok": True, "stop_order_id": stop_order_id, "trigger_price": trig,
                        "stop_method": "STP 市价止损单（aux_price 触发后市价成交）",
