...
## 双源对比与选用（2026-07-15 全维度实测，盯盘数据默认富途 + 老虎）

盯盘行情数据**走富途 + 老虎两家**。2026-07-15 全维度实测：**资金流富途更丰富**（分布含 super 超大单 + 分钟序列 + 十大买卖经纪）、**分钟 K 富途深 5.5 年**、**实时性富途 / 老虎是 WebSocket 毫秒级**。一句话：**盯盘数据走富途 + 老虎**。

| 数据维度 | 富途 OpenD | 老虎 SDK(TBNZ) | 默认用谁 / 谁最强 |
|---|---|---|---|
| 港股盘口 depth | ✅ 10 档 + 经纪队列 | ✅ 10 档（无经纪队列） | 富途（带经纪队列） |
| **资金流 capital** | ✅ **分布含 super 超大单 + 分钟序列 + 十大买卖经纪** | ✅ 分布(大中小单) + flow 序列 | **富途最丰富，盯盘默认走富途** |
| 经纪商 | ✅ broker queue + 十大买卖经纪 | ✅ stock_broker | 富途 |
| 行情 / 快照 quote | ✅ 快照 142 字段 | ✅ briefs | 富途（字段最全） |
| K 线（日 K） | ✅ ~20 年 | ✅ | 相当 |
| **分钟 K 历史** | ✅ **5.5 年（2021 起）** | — | **富途远优** |
| PE / PB / 换手 / 量比 | ✅ 快照含全部 | — | 富途 |
| 板块 / 产业链 / 研报 / 股东 / 做空 | ✅ 富途独有 | — | 富途独有 |
| 美股盘前 / 盘后 / 夜盘涨跌榜 | ✅ 富途独有 | ❌ TBNZ 无美股权限 | 富途 |
| 市场情绪 | ✅ 涨跌分布（更细） | ❌ | 富途 |
| **美股行情** | ✅ 10 档 + 资金流（实测 ret=0） | ❌ **TBNZ 无权限** | **富途单源** |
| 实时性 | **WebSocket 毫秒级** | **WebSocket 毫秒级** | 富途 / 老虎更快 |
| 成本 | 免费 | 免费（实测可用） | — |
| **交易下单 / 持仓 / 对账单** | ❌ 仅数据 | ❌ 仅数据 | 用户自决券商 App |
