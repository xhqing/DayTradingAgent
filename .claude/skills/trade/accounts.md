...
两实盘**资金可随时互划**。用哪个账户由**用户自决**（2026-07-15 起 AI 不管账户、不核实买力，见 SKILL「自主演练模式升级」第 6 条；账户差异参考见 SKILL「实盘账户对比」）。
...
**账户由用户自决（2026-07-15 升级第 6 条，取代旧「实盘账户由 AI 决定」）**：AI **不管账户、不写入账户字段、不核实买力 / 融资 / 做空额度**，演练模式假设执行、不查 `positions`。用哪个账户（主账户 / 日内融 / 模拟盘 / 其它券商）由用户自决、在自家 App 执行；AI 只用长桥主账户的行情 / 数据盯盘分析。两实盘差异（日内融收盘强平 vs 主账户不强平）见 SKILL「实盘账户对比」，供用户自决参考。

## 主账户资金状况

> 净资产、买力、可用港币/美元、融资授信等数值见 `accounts.json`（已 gitignore），**用户自决账户时用 `longbridge assets` 实时核实**（资金随时变化，勿直接引用快照数字；2026-07-15 起 AI 不核实资金）。

- 买力近零时主账户下不了单，需等资金到账或用日内融账户。仓位由"`risk.max_loss_per_trade` ÷ 每股损失"反推作风控建议；2026-07-15 起买力 / 账户由用户自决，AI 不核实（升级第 6 条）。
- **2026-07-15 盘中实测：主账户 max-qty buy 全返 0**（07709/02800 盈富/00700 腾讯 的 Cash Max Qty=0 且 Margin Max Qty=0）。虽 assets 显示融资授信 Max Finance 117k USD，但净资产仅 ~26.73 USD、买力 26.73 USD + 港币 ~209.5，**实际融资可买力为 0**（授信资格 ≠ 实际可用买力，受净资产约束）。**主账户买不了 1 手任何港股**——供用户自决账户时参考（主账户仅适合作行情 / 数据登录；若要实盘下单港股，用户宜选日内融账户）。
...
> 净资产、买力、可用港币/美元、融资授信等数值见 `accounts.json`（已 gitignore），**用户自决账户时用 `longbridge assets` 实时核实**。日内融买力含杠杆、是用户实盘下单的主要可用账户（主账户买力近零）。
...
- **强制平仓约束**：日内融持仓收盘前 15 分钟被券商强平，但 CLI 查不到（assets/portfolio 无相关字段）。港股 **15:45 HKT** 强平；美股美东 15:45 = **北京夏令时 03:45 / 冬令时 04:45** 强平。⚠️ 主账户**不强平**（日内须主动平，从不过夜）。账户差异与"看好设止损睡"策略见 SKILL「实盘账户对比」。

## 做空权限（实测）

- ⚠️ **2026-07-15 矛盾待实测**：用户称「主账户部分港股可做空」，与下方 2026-07-08 实测「主账户港股账户级不可空（603301）」矛盾。可能主账户后来开通融券、或 ETF 与个股有差异。暂按 skill 默认「假设可空」发做空信号，待实测确认。
- 港股账户级不可做空（2026-07-08 实测：模拟盘 + 真实主账户均报 603301，含盈富 ETF/腾讯）。
- 美股账户级可做空（TSLA 远价单 order sell 成功并秒撤，Order ID 1257651500528758784）。
- **CLI 查做空的方式与限制（2026-07-15 实测）**：`max-qty --side short` **无返回/不支持**（buy 侧返 Cash/Margin Max Qty 键值对，short 侧空）；`short-positions`/`short-trades` 是市场统计非权限接口。**结论：CLI 无只读取做空额度/权限的接口**，判定做空只能远价实单测试（卖单价远离市价、下单成功立刻 cancel）。但**信号模式禁下单**，故演练中按 skill 默认「假设可空」发做空信号、用户执行时 App 拦不可空标的。用户要求「AI 自己查可空」受此 CLI 限制，无法绕过。
- 日内融做空权限待开盘测。
